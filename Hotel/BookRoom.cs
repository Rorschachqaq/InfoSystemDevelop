...
namespace Hotel
{
    public partial class BookRoom : Form
    {
        ...
        public BookRoom()
        {
            ...
            ds = DBConnect.ExecuteQuery("select RoomNumber as '房间号',RoomType as '房间类型',RoomPrice as '房间价格',RoomStatus as '房间状态',Remarks as '房间说明' from RoomInfo where RoomStatus='available'");
        ...
        private void btnBook_Click(object sender, EventArgs e)
        {
            ...
            //DB操作
            if (DBConnect.Execute(@"insert into BookInfo(CustomerName,Phone,Deposit,CheckInTime,Days,RoomPrice,RoomType,RoomNumber,Remarks) values(
            '" + customerName + "','" + phone + "','" + deposit + "','" + checkInTime + "','" + days + "','" + roomPrice + "','" + roomType + "','" + roomNumber + "','" + remarks + "')") > 0)
            {
                if (DBConnect.Execute("update RoomInfo set RoomStatus='reserved' where RoomNumber='" + Convert.ToInt32(txtRoomNumber.Text.Trim()) + "'") > 0)
                {
