...
namespace Hotel
{
    public partial class CancelReservation : Form
    {
        ...
        {
            InitializeComponent();
            ds = DBConnect.ExecuteQuery("select ID as 'ID',CustomerName as '顾客姓名',Phone as '联系电话',Deposit as '押金',CheckInTime as '入住时间',Days as '预计天数',RoomPrice as '房间价格',RoomType as '房间类型',RoomNumber as '房间号码',Remarks as '备注' from BookInfo");
        ...
        private void btnCancelReservation_Click(object sender, EventArgs e)
        {
            ...
            if (DBConnect.Execute("delete from BookInfo where ID='" + iD + "'") > 0&& DBConnect.Execute("update RoomInfo set RoomStatus='available' where RoomNumber='"+Convert.ToInt32( txtRoomNumber.Text.Trim())+"'")>0)
            {
        ...
        private void btnAllClear_Click(object sender, EventArgs e)
        {
            ...
            {
                if (DBConnect.Execute("update RoomInfo set RoomStatus='available' where RoomStatus='reserved'") > 0 && DBConnect.Execute("delete from BookInfo where 1=1") > 0)
                {
