...
namespace Hotel
{
    public partial class CheckIn : Form
    {
        ...
        private void CheckIn_Load(object sender, EventArgs e)
        {
            ...
            ds = DBConnect.ExecuteQuery("select RoomNumber as '房间号',RoomType as '房间类型',RoomPrice as '房间价格',RoomStatus as '房间状态',Remarks as '房间说明' from RoomInfo where Roomstatus<>'occupied'");
        ...
        private void dataGridView1_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (dataGridView1.SelectedRows[0].Cells[3].Value.ToString() == "reserved")
            {
               DataSet tempDs = DBConnect.ExecuteQuery("select * from BookInfo where RoomNumber='"+dataGridView1.SelectedRows[0].Cells[0].Value.ToString()+"'");
        ...
        private void dataGridView2_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            RoomStatus = "available";
            //如果房间已经被预订，则显示预定客人的信息
            if (dataGridView2.SelectedRows[0].Cells[3].Value.ToString() == "reserved")
            {
                DataSet tempDs = DBConnect.ExecuteQuery("select * from BookInfo where RoomNumber='" + dataGridView2.SelectedRows[0].Cells[0].Value.ToString() + "'");
                txtCustomerName.Text = tempDs.Tables[0].Rows[0][1].ToString();
                txtPhone.Text = tempDs.Tables[0].Rows[0][2].ToString();
                RoomStatus = "reserved";
            }
        ...
        private void btnCheckIn_Click(object sender, EventArgs e)
        {
            ...
            //DB操作
            if (DBConnect.Execute(@"insert into CustomerInfo(CustomerName,Sex,CredentialNumber,Phone,Deposit,CheckInTime,Days,RoomPrice,RoomType,RoomNumber,Remarks) values(
            '" + customerName + "','" + sex + "','" + credentialNumber + "','" + phone + "','" + deposit + "','" + checkInTime + "','" + days + "','" + roomPrice + "','" + roomType + "','" + roomNumber + "','" + remarks + "')") > 0)
            {
                if (DBConnect.Execute("update RoomInfo set RoomStatus='occupied' where RoomNumber='" + roomNumber + "'") > 0)
                {
                    MessageBox.Show("入住成功！");
                    if (RoomStatus == "reserved")
                    {
                        DBConnect.Execute("delete from BookInfo where RoomNumber='" + roomNumber + "'");
                    }
                    //刷新datagridview1
                    ds = DBConnect.ExecuteQuery("select RoomNumber as '房间号',RoomType as '房间类型',RoomPrice as '房间价格',RoomStatus as '房间状态',Remarks as '房间说明' from RoomInfo where Roomstatus<>'occupied'");
                    dataGridView1.DataSource = ds.Tables[0].DefaultView;
        ...
        private void btnBookCus_Click(object sender, EventArgs e)
        {
            ...
            //判断查询的结果
            DataSet tempDs = DBConnect.ExecuteQuery(sql);
            ...
            dataGridView2.DataSource = DBConnect.ExecuteQuery("select RoomNumber as '房间号',RoomType as '房间类型',RoomPrice as '房间价格',RoomStatus as '房间状态',Remarks as '房间说明' from RoomInfo where RoomNumber in("+sql+")").Tables[0].DefaultView;
            //转到用户已预订房间的界面
            btnSwich.Text = ">>";
