...
namespace Hotel
{
    public partial class HotelManage : Form
    {
        ...
        private void HotelManage_SizeChanged(object sender, EventArgs e)
        {
            ...
            LoadHouseMap(tabPage1, "1=1");  // 使用新的查询字符串
            LoadHouseMap(tabPage2, "RoomType='标准单人间'");
            LoadHouseMap(tabPage3, "RoomType='标准双人间'");
            LoadHouseMap(tabPage4, "RoomType='豪华单人间'");
            LoadHouseMap(tabPage5, "RoomType='豪华双人间'");
            LoadHouseMap(tabPage6, "RoomType='商务套房'");
            LoadHouseMap(tabPage7, "RoomType='总统套房'");
        }
        ...
        {
            DataSet ds = DBConnect.ExecuteQuery("select * from RoomInfo where 1=1 and " + addsql);
            ...
            for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
            {
                ...
                //添加图片
                if (ds.Tables[0].Rows[i][3].ToString() == "occupied")
                {
                    btnArr[i].Image = redHouseImage;
                }
                else if (ds.Tables[0].Rows[i][3].ToString() == "reserved")
                {
