...
namespace Hotel
{
    public partial class CheckOut : Form
    {
        ...
        {
            dataGridView1.DataSource = DBConnect.ExecuteQuery("select ID,CustomerName as '宾客姓名',Sex as '性别',CredentialNumber as '身份证号',Phone as '手机号',Deposit as '押金',CheckInTime as '入住时间',Days as '预计天数',RoomPrice as '房间价格',RoomType as '房间类型',RoomNumber as '房间号码',Remarks as 备注 from CustomerInfo").Tables[0].DefaultView;
        }
        ...
        private void btnCheckOut_Click(object sender, EventArgs e)
        {
            ...
            if (DBConnect.Execute("insert into Record(CustomerName,Sex,CredentialNumber,Phone,CheckInTime,CheckOutTime,Days,Spending,RoomType,RoomNumber,Remarks) values('"
                +custormerName+"','"+sex+"','"+credentialNumber+"','"+phone+"','"+checkInTime+"','"+checkOutTime+"','"+days+"','"+spending+"','"+roomType+"','"+roomNumber+"','"+remarks+"')") > 0)
            {
                //进入代表记录历史记录成功
                if (DBConnect.Execute("delete from CustomerInfo where ID='" + id + "'") > 0)
                {
                    //进入代表删除宾客成功
                    if (DBConnect.Execute("update RoomInfo set RoomStatus='available' where RoomNumber='" + roomNumber + "'") > 0)
                    {
                        //进入代表更改房间状态成功
