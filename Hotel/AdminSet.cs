...
namespace Hotel
{
    public partial class AdminSet : Form
    {
        ...
        public AdminSet(string user)
        {
            ...
            dataGridView1.DataSource = DBConnect.ExecuteQuery("select UserName as '用户名',UserType as '用户类型' from UserInfo").Tables[0];
        }
        ...
        private void btnAlter_Click(object sender, EventArgs e)
        {
            ...
            if (DBConnect.Execute(sql)>0)
            {
                ...
                dataGridView1.DataSource = DBConnect.ExecuteQuery("select UserName as '用户名',UserPassword as '密码',UserType as '用户类型' from UserInfo").Tables[0];
        ...
        private void btnDel_Click(object sender, EventArgs e)
        {
            ...
            string usertype = cboType.Text;
            if (User == DBConnect.ExecuteQuery("select * from UserInfo where UserName='" + username + "'").Tables[0].Rows[0][0].ToString())
            {
                MessageBox.Show("不删除本用户！");
                return;
            }
            if (DBConnect.Execute("delete from UserInfo where UserName='" + username + "'") > 0)
            {
